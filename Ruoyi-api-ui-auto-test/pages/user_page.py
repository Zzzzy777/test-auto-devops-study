...
class UserPage:
    ...
    def search_username(self, username: str) -> None:
        ...
        search_button = self.page.locator("button.el-button--primary.el-button--mini").first
    ...
    def cancel_add(self) -> None:
        """\u53d6\u6d88\u65b0\u589e\u7528\u6237\uff0c\u4e0d\u63d0\u4ea4\u8bf7\u6c42\u3002"""
        # RuoYi ??????? ??????????????????????
        cancel_button = self.add_dialog.locator("button.el-button--default").last
    ...
    def reset_search(self) -> None:
        """\u70b9\u51fb\u7528\u6237\u5217\u8868\u67e5\u8be2\u6761\u4ef6\u7684\u91cd\u7f6e\u6309\u94ae\u3002"""
        reset_button = self.page.locator("button").filter(
            has_text="\u91cd\u7f6e"
        ).first
        expect(reset_button).to_be_visible(timeout=8000)
        reset_button.click(force=True)
        self.page.wait_for_timeout(300)
    ...
    def delete_user(self, username: str) -> None:
        ...
        row = self.user_row(username)
        # ???????????????????????? DOM ???
        delete_button = row.locator("button.el-button--text").nth(1)
        expect(delete_button).to_be_visible(timeout=5000)
        delete_button.click(force=True)
        # Element UI ??????????? :visible ?????????????????
        message_box = self.page.locator(".el-message-box").last
        expect(message_box).to_be_visible(timeout=10000)
        # ????????????? primary ???
        confirm_button = message_box.locator("button.el-button--primary").last
        ...
        assert delete_response.ok, (
            f"??????????: {delete_response.status} {delete_response.url}"
        )
        # ???????????????????????????????
        remaining_rows = self.page.locator(
            ".el-table__body-wrapper tbody tr"
        ).filter(has_text=username)
        expect(remaining_rows).to_have_count(0, timeout=10000)
